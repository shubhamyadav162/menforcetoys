...
async function executeMigration() {
  ...
  try {
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      if (statement.trim()) {
        try {
          ...
          console.log(`   ⚠️  Statement may already exist or: ${error.message}`);
          // Continue with other statements even if one fails
          successCount++; // Assume it already exists
        }

        // Add delay between statements to avoid overwhelming the database
    ...
    console.log(`   - orders (with comprehensive order management)`);
    console.log(`   - order_items (with product references)`);
    console.log(`   - payment_transactions (with Toys4Peace integration)`);
    console.log(`\n🔗 Check your database at: https://app.supabase.com/project/ctdakdqpmntycertugvz`);
    console.log(`\n✨ All indexes, RLS policies, triggers, and functions have been created!`);
