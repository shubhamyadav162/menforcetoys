-- Fix for NP Wellness Store - Create missing orders table
-- This script creates the orders table that the application expects

-- Drop the existing orders_new table if it exists and rename to orders
DROP TABLE IF EXISTS orders CASCADE;

-- Create the orders table as expected by the application
...
CREATE TRIGGER update_order_items_updated_at BEFORE UPDATE ON order_items FOR EACH ROW EXECUTE FUNCTION update_orders_updated_at();
...
CREATE TRIGGER update_payment_transactions_updated_at BEFORE UPDATE ON payment_transactions FOR EACH ROW EXECUTE FUNCTION update_orders_updated_at();

-- Success message
DO $$
BEGIN
    RAISE NOTICE 'Orders table and related tables created successfully!';
    RAISE NOTICE 'Tables created: orders, order_items, payment_transactions';
